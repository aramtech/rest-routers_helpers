...
export const describe = lock_method(
    (options: DescriptionProps) => {
        ...
        try {
            ...
            let route_precise_path = path.join(
            ...
            let route_precise_path_with_slash_at_the_end;
            if (route_precise_path == "/") {
                route_precise_path_with_slash_at_the_end = "/";
            } else {
                if (route_precise_path.endsWith("/")) {
                    route_precise_path_with_slash_at_the_end = route_precise_path;
                    route_precise_path = route_precise_path.slice(0, -1);
                } else {
                    route_precise_path_with_slash_at_the_end = route_precise_path + "/";
                }
            }
...
<!-- --end-- ${route_precise_path} -->`;
            ...
            descriptions_map[route_precise_path_with_slash_at_the_end] = {
                ...options,
                fileUrl: route_precise_path_with_slash_at_the_end,
            };
        } catch (error) {
            console.error(error);
